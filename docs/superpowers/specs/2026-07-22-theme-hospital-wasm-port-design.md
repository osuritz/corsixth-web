...
1. **"Play the demo"** — AMENDED 2026-07-22 after the M2 CORS spike: archive.org's file servers send no CORS headers (preflight → 405), so in-browser fetch is impossible without a proxy, and a proxy would violate this spec's never-host-or-proxy posture. Sponsor decision: **two-click demo** — the button links the user to the direct archive.org download (`https://archive.org/download/HOSP_zip/HOSP.zip`, verified byte-exact against the engine's demo checks: `DataM/Demo.dat` present, `LEVEL.L1`=163948B, `BUTTON01.DAT`=252811B, `WINLEVEL.SMK`=243188B), and the user drops the downloaded zip on the page. We never host, proxy, or bundle EA-copyrighted bytes — zero bytes transit our infrastructure, identical posture to drag-drop. Fallback item identifiers if `HOSP_zip` disappears: `ThemeHospital_1020`, `theme-hospital-demo-1996` (re-verify contents before switching).
